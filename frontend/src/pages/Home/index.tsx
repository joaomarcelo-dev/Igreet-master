import './style.scss';

import CardOptionsPages from "../../components/CardOptionsPages"
import listOfCardsPages from "../../utils/listOfCardsPages.utils"
import { IoMdNotificationsOutline } from 'react-icons/io';
import { FaRegCalendarAlt } from 'react-icons/fa';

const defalImgProfile = 'data:image/png;base64,';

export default function Home() {
  return (
    <>
      <header
        className='header-home flex_center'
      >
        <div className='content-info-profile-home flex_center'>
          <img
            className='img-profile-home'
            src={ defalImgProfile }
          />

          <h5 className='title-name-home'>
            Olá,
            {'\n'}
            <span>João Marcelo</span>
          </h5>
        </div>

        <button
          className='icon-notification-header-home'
        >
          <IoMdNotificationsOutline
            size={25}
          />
        </button>

      </header>
      <section
        className="content-cards-options-pages-grid"
      >

        <h6>Acesso Rápido</h6>

        <div className='container-buttons-access-speed flex_center'>
          {
            listOfCardsPages.map((card) => {
              return (
                <CardOptionsPages
                  key={card.id}
                  title={card.title}
                  uri={card.uri}
                  active={card.active}
                  Icon={card.icon}
                />
              )
            })
          }

        </div>
      </section>
      
      <div className='conten-button-agent-new-cosult-manual flex_center'>
        <button className='button-agend-new-consult-manual'>
          <FaRegCalendarAlt size={ 25 } className='icon-calendar-home' />
          {' '}
          Agendar uma consulta manualmente
        </button>
      </div>
    </>
  )
}